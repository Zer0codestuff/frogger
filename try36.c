...
void generate_thread(int, int, int);
void generate_frog();
void generate_crocs();
void generate_plants();
void generate_threades();
void send_signal(pthread_t pid);
void pause_thread(pid_t);
void kill_thread(pthread_t);
void kill_all();
int which_plant(int);
bool check_bull_collisions(pthread_t, int, int, int);
...
void inefficient_cleanup(pthread_t);
...
void* plant(void* nn) {
   ...
   while(true) {
      ...
      shoot = rand() % 5;
      if (shoot == 3) {
         generate_thread(BULL_PL_ID, -1, m.x);   
      }
      usleep(PLANT_DELAY);    
      }  
}
...
void* frog() {
    ...
    struct msg m; struct screen sfrog1; int input; bool new_input = false; 
    
    m.id = FROG_ID;
    sfrog1.x = FROG_STARTX;
    sfrog1.y = FROG_STARTY;
    m.pid = pthread_self();
    m.x = 0;
    m.y = 0;
    while(true) {
...
//template per generare threadi
void generate_thread(int id, int flusso, int n_plant) {
   pthread_t tid; struct croc_params cparams; struct bull_params bparams;
      ...
      switch(id) {
         ...
         case FPLANT_ID:
            pthread_create(&tid, NULL, &plant_creator, NULL);
            break;
         ...
         case EV_CROC_ID:
           cparams.flusso = flusso;
           cparams.evil = true;
           pthread_create(&tid, NULL, &croc, (void*)&cparams);
           usleep(1000);
           break;
            
         case PLANT_ID:
            pthread_create(&tid, NULL, &plant, &n_plant);
            usleep(1000);
            break;
            
         case BULL_ID:
            bparams.x = sfrog.x+1;
            bparams.y = sfrog.y - BULL_Y;
            bparams.type = -1;
         ...
         case BULL_PL_ID:
            bparams.x = n_plant;
            bparams.y = FIRST_PLANT_PY+PLANT_Y;
            bparams.type = 1;
            pthread_create(&tid, NULL, &bullet, &bparams);
            usleep(1000);
            break;
...
//genera il thread rana
void generate_frog() {
   generate_thread(FROG_ID, -1, -1);
}

//genera il thread che genererà i thread coccodrillo
void generate_crocs() {
   generate_thread(FCROC_ID, -1, -1);
}

//genera il thread che genererà i thread pianta
void generate_plants() {
   generate_thread(FPLANT_ID, -1, -1);
}

//funzione che genera i thread
void generate_threades() {
   generate_crocs();
   generate_frog();
   generate_plants();
...
//Uccide ogni thread presente in gioco (tranne se stesso)
void kill_all() {
   pthread_t arr[500] = {0}; int count = 0; bool not_killed = true;
   for (size_t i = 0; i < CROC_X*2 + DIM_X; i++) {
      for (size_t j = 0; j < DIM_Y; j++) {
         for (size_t z = 0; z < 500; z++) {
         if (pthread_equal(arr[z], game_matrix[i][j].pid)) {not_killed = false;}
         }
         if (not_killed) {
            kill_thread(game_matrix[i][j].pid);
            usleep(2000);
            arr[count] = game_matrix[i][j].pid;
            count += 1;
            not_killed = true;
            }
      }
   }
   kill_thread(fcroc_pid);
   kill_thread(fplant_pid);
}
...
//pausa il thread: utilizzata quando le piante vengono colpite da un proiettile
void pause_thread(pid_t pid) {
   kill(pid, SIGSTOP);
   //aspetto che il threado sia in pausa
   waitpid(pid, NULL, WUNTRACED);  
}

//Uccide il thread
void kill_thread(pthread_t tid)  {
   if (!(pthread_equal(tid, -1) || pthread_equal(tid, -2))) {
...
bool check_bull_collisions(pthread_t pid, int x, int y, int speed) {
   bool flag = false; struct msg m;
   if (speed > 0) {
      if (game_matrix[x][y+BULL_Y-1].id == BULL_ID) {
         kill_thread(pid);
         kill_thread(game_matrix[x][y+BULL_Y-1].pid);
         m.x = x;
         m.id = BULL_ID;
         m.y = y;
      ...
      if (game_matrix[x][y].id == BULL_PL_ID) {
         kill_thread(pid);
         kill_thread(game_matrix[x][y].pid);
         m.x = x;
         m.id = BULL_ID;
         m.y = y;
         delete_old(entity_data[BULL_ID], m);
     ...
     else if (game_matrix[x][y].id == EV_CROC_ID || game_matrix[x][y+1].id == EV_CROC_ID || game_matrix[x][y+2].id == EV_CROC_ID || game_matrix[x][y+3].id == EV_CROC_ID) {
         kill_thread(pid);
         send_signal(game_matrix[x][y].pid);
         m.x = x;
         m.y = y+speed;
...
bool is_frog_on_croc() {
   pthread_t old = game_matrix[sfrog.x][sfrog.y].pid; pthread_t new = game_matrix[sfrog.x][sfrog.y].pid;
...
//cancella le celle con tid inattivi
void inefficient_cleanup(pthread_t tid) {
   for (size_t i = 0; i < DIM_Y; i++) {
      for (size_t j = 0; j < DIM_X + 2*CROC_X; j++) {
         if (pthread_equal(game_matrix[j][i].pid, tid))
...
bool is_out_of_bounds(struct msg m) {
   ...
   if (m.id == FROG_ID && (sfrog.y > MARCIAPIEDESY + MARCIAPIEDE_Y - FROG_Y || sfrog.x < CROC_X+1 || sfrog.x > FIUMESX + FIUME_X + CROC_X-FROG_X-2)) {
      partita.manche = false;
      //partita.loss = true;
      //kill_thread(sfrog.pid);
      inefficient_cleanup(m.pid);
      return true;      
   }
...
void update(struct msg e, struct msg m){
   ...
   if (m.id == FROG_ID && (m.x != 0 || m.y != 0)) {sfrog.on_croc = is_frog_on_croc(); 
      if (sfrog.on_croc) {sfrog.pid = game_matrix[sfrog.x][sfrog.y+2].pid; }}
   ...
   if (m.id == BULL_ID || m.id == BULL_PL_ID) {
      ...
      if ((m.id == BULL_ID) || (game_matrix[j][i].id != CROC_ID && game_matrix[j][i].id != EV_CROC_ID)){
...
struct manchestr game(bool manche, long int startingtime, int vite, int score) {
   ...
   while(partita.manche) {
      ...
      //update schermo 
      //il mutex è necessario per via del refresh di getch()
      pthread_mutex_lock(&mutex_refresh);
      wrefresh(gamewin);
      pthread_mutex_unlock(&mutex_refresh);
      //pausa
